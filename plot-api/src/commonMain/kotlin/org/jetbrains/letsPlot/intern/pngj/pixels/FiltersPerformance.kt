...
package org.jetbrains.letsPlot.intern.pngj.pixels

import org.jetbrains.letsPlot.intern.pngj.FilterType
import org.jetbrains.letsPlot.intern.pngj.ImageInfo
import org.jetbrains.letsPlot.intern.pngj.PngHelperInternal
import org.jetbrains.letsPlot.intern.pngj.PngjExceptionInternal
import org.jetbrains.letsPlot.intern.pngj.utils.arraycopy
import org.jetbrains.letsPlot.intern.pngj.utils.fill
import kotlin.math.ln
import kotlin.math.pow

/** for use in adaptative strategy  */
class FiltersPerformance(imgInfo: ImageInfo) {
    ...
    private fun init() {
        if (filterWeights[0] < 0) { // has not been set from outside
            arraycopy(FILTER_WEIGHTS_DEFAULT, 0, filterWeights, 0, 5)
    ...
    private fun updateFromRawOrFiltered(
        ...
        if (!initdone) init()
        if (rown != lastrow) {
            fill(absum, Double.NaN)
            fill(entropy, Double.NaN)
        }
        lastrow = rown
    ...
    fun setFilterWeights(weights: DoubleArray) {
        arraycopy(weights, 0, filterWeights, 0, 5)
    }
