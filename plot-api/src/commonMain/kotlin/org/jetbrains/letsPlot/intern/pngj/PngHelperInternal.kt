...
object PngHelperInternal {
     ...
     */
    fun readInt2(inputStream: InputStream): Int {
        return try {
     ...
     * PNG uses "network byte order"
     */
    fun readInt4(inputStream: InputStream): Int {
        return try {
            val b1: Int = inputStream.read()
            val b2: Int = inputStream.read()
    ...
    fun writeByte(os: OutputStream, b: Byte) {
        try {
            os.write(b.toInt())
        } catch (e: Throwable) {
    ...
    fun writeByte(os: OutputStream, bs: ByteArray) {
        try {
            os.write(bs)
        } catch (e: Throwable) {
            throw PngjOutputException(e)
    ...
    fun writeInt2(os: OutputStream, n: Int) {
        val temp = byteArrayOf((n shr 8 and 0xff).toByte(), (n and 0xff).toByte())
        writeBytes(os, temp)
    }

    fun writeInt4(os: OutputStream, n: Int) {
     ...
     */
    fun readBytes(inputStream: InputStream, b: ByteArray, offset: Int, len: Int) {
        if (len == 0) return
        try {
            var read = 0
            while (read < len) {
    ...
    fun skipBytes(inputStream: InputStream, len: Long) {
    ...
    fun writeBytes(os: OutputStream, b: ByteArray) {
        try {
            os.write(b)
        } catch (e: Throwable) {
            throw PngjOutputException(e)
    ...
    fun writeBytes(os: OutputStream, b: ByteArray, offset: Int, n: Int) {
        try {
            os.write(b, offset, n)
        } catch (e: Throwable) {
            throw PngjOutputException(e)
