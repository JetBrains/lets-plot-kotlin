...
import org.jetbrains.letsPlot.intern.pngj.chunks.ChunksList
...
object TestSupport {
    ...
    fun showChunks(chunks: List<PngChunk?>): String {
    ...
    @JvmOverloads
    fun feedFromStreamTest(`as`: ChunkSeqReader, fs: String, bufferSize: Int = -1) {
        @Suppress("NAME_SHADOWING")
        var bufferSize = bufferSize
        if (bufferSize < 1) bufferSize = randBufSize()
        val bf = BufferedStreamFeeder(fs.toInputStream(), bufferSize)
        while (!`as`.isDone) {
    ...
    fun chunkListToChunksRaw(chunks: List<PngChunk?>): List<ChunkRaw> {
    ...
    fun showFilters(
        ...
        val png = PngReaderByte(pngr.toInputStream())
        val sb = StringBuilder()
    ...
    @JvmOverloads
    fun showLine(linex: IImageLine, maxtoshow: Int = 9): String {
     ...
     * @return
     */
    fun istream(f: String): InputStream {
        return File(f).toInputStream()
    }
     ...
     * @return
     */
    fun ostream(f: String): OutputStream {
     ...
     * @return
     */
    fun istream(f: File): InputStream {
        @Suppress("NAME_SHADOWING")
        var f = f
        return try {
     ...
     * @return
     */
    fun ostream(f: File): OutputStream {
        @Suppress("NAME_SHADOWING")
        var f = f
        return try {
            if (!f.isAbsolute) f = File(resourcesDir, f.path)
    ...
    fun testSameCrc(ori: File, dest: File) {
        ...
        val png1 = PngReader(ori.toInputStream())
        PngHelperInternal.initCrcForTests(png1)
        val png2 = PngReader(dest.toInputStream())
    ...
    fun testSameValues(ori: File, dest: File, tolerance: Int = 0) {
        val png1 = PngReaderInt(ori.toInputStream())
        val png2 = PngReaderInt(dest.toInputStream())
    ...
    fun testCrcEquals(image1: File, crc: Long) {
        val png1 = PngReader(image1.toInputStream())
        PngHelperInternal.initCrcForTests(png1)
        png1.readRow(png1.imgInfo!!.rows - 1)
        png1.end()
        val crc1 = PngHelperInternal.getDigest(png1)
    ...
    fun createNullOutputStream(): NullOutputStream {
        return NullOutputStream()
    }

    fun getChunkById(id: String?, chunks: Collection<PngChunk?>): List<PngChunk> {
    ...
    fun readAllChunks(file: File, includeIdat: Boolean): ChunksList {
        val pngr = PngReader(InputStream(file.readBytes()))
    ...
    @JvmOverloads
    fun prepareFileTmp(f: File, palette: Boolean = false): PngWriter {
        return prepareFileTmp(f, ImageInfo(32, 32, 8, alpha = false, greyscale = false, indexed = palette))
    }

    fun generateNoiseLine(imi: ImageInfo?): IImageLine { // byte format!
    ...
    fun endFileTmp(png: PngWriter) { // writes dummy data
    ...
    fun getImageInfo(f: File): ImageInfo? {
        val p = PngReaderByte(f.toInputStream())
        p.close()
        return p.imgInfo
    }
    ...
    private fun File.toInputStream() = InputStream(readBytes())
    private fun String.toInputStream() = InputStream(this.toByteArray())

    private class FileOutputStream(private val file: File) : OutputStream() {
